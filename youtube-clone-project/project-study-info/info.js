...
// # 14.0
  - webm 파일은 영상길이가 없고 움직임도 없다.
  - FFmpeg를 이용하여 webm에서 mp4로 변환하고, 비디오의 썸네일을 추출한다.
  - FFmpeg는 백엔드에서 실행해야 한다. 이것은 비용이 든다.
  - WebAssembly는 웹사이트가 매우 빠른 코드를 실행할 수 있게 해준다. 실행비용이 큰 프로그램들을 브라우저에서 실행할 수 있다.
  - 두가지를 결합한다. ffmpeg.wasm
  - ffmpeg.wasm는 비디오를 변환하기 위해 서버가 아닌, 사용자의 컴퓨터를 사용한다. ffmpeg는 C로 만들어졌으나 웹어셈블리를 이용하여 브라우저에서 실행시킬 수 있다.
  - 우리가 webm에서 mp4로 바꾸는 이유는 모든 기기가 webm을 이해하는 것은 아니기 때문이다.
